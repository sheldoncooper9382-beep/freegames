...
isDealing, hasCelebrated, audioCtx, audioUnlocked;
...
function init() {
    ...
    dealCards();
    render();
    // Only animate the initial deal once per new game
    isDealing = false;
}
...
function renderStacks() {
    ...
    stacks.forEach((stack, sIdx) => {
        ...
        stack.forEach((card, cIdx) => {
            ...
            if (isDealing) {
                cardEl.className += ' deal-in';
                const dealIndex = (sIdx * (sIdx + 1)) / 2 + cIdx;
                cardEl.style.animationDelay = `${Math.floor(dealIndex * 30)}ms`;
            }
...
function handleClick(evt) {
    ...
    if (!audioUnlocked && clickDest !== 'resetButton') {
        unlockAudio();
    }
...
/* ----- enhancements: sound + deal + win celebration ----- */
...
function unlockAudio() {
    try {
        ...
        // Tiny silent tick to fully unlock in some browsers
        ...
        // If audio can't be initialized, fail silently (game still works)
        audioUnlocked = true;
    }
}

// A short "card slap" synthesized with noise + a quick transient.
...
function playCardSlap() {
    ...
    try {
        ...
        for (let i = 0; i < bufferSize; i++) {
            // weighted noise: sharper at start, softer tail
...
function showWinCelebration() {
    // Prevent duplicates
    if (document.getElementById('win-celebration')) return;

    const overlay = document.createElement('div');
    overlay.id = 'win-celebration';
    ...
    for (let i = 0; i < pieces; i++) {
        ...
        c.style.animationDuration = `${2.8 + Math.random() * 1.2}s`;
